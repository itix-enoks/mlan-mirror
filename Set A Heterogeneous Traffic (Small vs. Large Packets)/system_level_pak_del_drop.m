...
%    for 802.11ac tdma: add queuing delay = (position_in_round - 1) x slot_dur
%    for 802.11ax ofdma: no queuing - all users transmit in the same slot

clear; clc; close all;

% traffic profiles - set a
N_USERS = 4;
...
%    queuing_delay  = (u-1) x slot_dur_ac(u)   [wait for preceding users]
...
% 802.11ax - he-mu per user, ofdma time model
%
%  delay breakdown:
...
for u = 1:N_USERS
    ...
    avg_delay_ac(u) = mean(rec_ac(u).delay(ok_ac)) * 1e6;
...
for u = 1:N_USERS
    all_delays_ac = [all_delays_ac, rec_ac(u).delay(~rec_ac(u).dropped)];
    all_delays_ax = [all_delays_ax, rec_ax(u).delay(~rec_ax(u).dropped)];
end
